...
bool FocalSimReader::outfile_open(){
  out_tfile = std::make_unique<TFile>(out_filename().c_str(), "RECREATE");
...
void FocalSimReader::update_out_ttree_event(){
  ...
  *generic_event.fractions = fractions;
  //*generic_event.label = std::vector<General::float_type>(generic_event.x->size(), General::float_type>(1, 1.0f);
}
